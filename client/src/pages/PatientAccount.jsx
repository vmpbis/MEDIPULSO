...
function PatientAccount() {
  ...
  // Optional: Google sign-in flow pasted here (now fully wired so it compiles)
  const handleGoogleClick = async () => {
    try {
      ...
      const firstName = profile?.given_name ?? (display.split(" ")[0] || "");
      const lastName  = profile?.family_name ?? (display.split(" ").slice(1).join(" ") || "");
